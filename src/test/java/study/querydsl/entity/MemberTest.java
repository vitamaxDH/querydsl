...
//                .where(member.username.eq(team.name))
    ...
    public void findUserDto () throws Exception {
        ...
        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class,
                        member.username.as("name"),     // field 에서 이름이 맞지 않을 경우
                        ExpressionUtils.as(JPAExpressions   // 서브 쿼리는 어쩔 수 없다.
                                .select(memberSub.age.max())
                                .from(memberSub), "age")
                ))
                .from(member)
    ...
    // ex) 광고 상태 isValid, 날짜가 IN : isServiceable 등으로 공통 조건을 뽑아낼 수 있다.

    private BooleanExpression usernameEq(String usernameCond, Integer ageCond) {
